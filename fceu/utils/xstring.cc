...
std::vector<string> tokenize_str(const string & str,
                                      const string & delims=", \t")
{
  using namespace std;
  using ::string;
...
#include "fceu/ConvertUTF.h"
namespace UtfConverter
{
    std::wstring FromUtf8(const string& utf8string)
    {
    ...
    string ToUtf8(const std::wstring& widestring)
    {
        ...
        if (sizeof(wchar_t) == 2)
        {
            ...
            *targetstart = 0;
            string resultstring(utf8stringnative);
            delete [] utf8stringnative;
            return resultstring;
        }
...
namespace UtfConverter
{
    void SeqValue(string& result, unsigned n)
    {
        if(n < 0x80)                 // <=7 bits
            result += (char)n;
        else
    ...
    unsigned DecData(const string& input, size_t& pos)
    {
    ...
    std::wstring FromUtf8(string& input) // string -> wstring
    {
        std::wstring result;
    ...
    string ToUtf8(std::wstring& input) // wstring -> string
    {
        string result;
        for(string::size_type pos = 0; pos < input.size(); ++pos)
...
//convert a string to std::wstring
std::wstring mbstowcs(string str) // UTF8->UTF32
{
	try {
...
//convert a std::wstring to string
string wcstombs(std::wstring str) // UTF32->UTF8
{
