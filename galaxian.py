...
class Game:
  def __init__(self):
    ...
    self._sock.connect(('localhost', 62345))
  ...
  def Step(self, action):
    ...
    frame.action = self.NextLine()
    assert frame.action == action or action == 'human', action + ' ' + frame.action
    frame.action_id = OUTPUT_IDS[frame.action]

    reward = float(self.NextLine())
    ...
    while True:
      ...
      if key == 'gx':
        ...
        assert key[0] == 'i' or key[0] == 'b', "key=" + key
        gid = int(key[1])
        ix, iy = map(int, key[3:].split(','))
        ix += WIDTH/2
